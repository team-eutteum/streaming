...
import { GENIE_CHART_CONTENT } from '@/lib/constants/chart.constants';
...
function ChartArea({ chartType }: { chartType: string }) {
  const { data, isLoading, isError } = useQuery<MusicChartContentProps[]>({
    queryKey: ['chart', 'genieChartData', chartType],
    queryFn: () => getData(`genie/${chartType}`),
    staleTime: 0,
  });
